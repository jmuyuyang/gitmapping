...
Git.diffLog = function(version,callback){
  var cmd = gitCommand.concat(["log","--pretty=format:author:%an,%ad:%s",version+"..."]);
  var logExec = child_process.spawn("git", cmd);
  Git.exec(logExec,callback);
}
