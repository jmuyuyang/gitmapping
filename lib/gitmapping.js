...
module.exports = {
	...
	init : function (source,remoteUrl,branch,callback){
		this.initGit(source);
		if(!this.check(source,remoteUrl)) return callback("the repository in /tmp/gitmapping/"+source+" is existed,please use -s redefined source name",null);
	...
	runWithFile: function(dir,names,updateRepo,callback){
		var parent = this;
		function runRule(name,callback){
			var rule = parent.rules[name];
			if(!rule) return callback("the rule "+name+" is not exists");
			if(!fs.existsSync("/tmp/gitmapping/"+rule.repo)) updateRepo = 1;
			var $task_list = [ 
				function(callback){
		...
		this.find_meta(dir,function(err,data){
			if(err) return callback(err);
			if(!names) names = Object.keys(parent.rules);
